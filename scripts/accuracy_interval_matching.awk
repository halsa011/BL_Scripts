...
# FNR is the local record number and NR is the global, and the condition is to perform array filling with first input (GPS/reference traj) file only
FNR==NR {
...
{
for (tod in LAT)
	if (sqrt((tod-$TOD_col)^2)<match_dt && FNR>1 && $Lat_col!=0 && $Lon_col!=0){
#Debug: print out lines with non matching time GPS-MLAT
# 	{}
# else
# print FNR
localcarth(LAT[tod],LON[tod], height_s)
...
NR=$NR_col
print NR, x_meas, y_meas, x_ref, y_ref, acc
